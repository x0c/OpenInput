...
本机已覆盖安装 1.1.4 (9)。找回窗已点到。设置 › 通用含检查更新与退出对等入口。公开更新三种网络结果与开机自启重新登录未测。改、评审或排查菜单栏、隐藏图标、开机自启、再次打开或恢复窗口前**必读** [菜单栏、隐藏图标与恢复窗口](docs/MENU_BAR_LIFECYCLE_GUIDE.md)（不读会把输入小窗当成恢复面，或隐藏图标后找不到应用）。禁止一次藏多个应用的图标。
