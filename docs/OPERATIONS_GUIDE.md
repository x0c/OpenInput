...
公开更新仓是首装和更新产物的唯一可访问入口：同一版本的 DMG 与 ZIP 都挂在该仓的 Release，更新清单从 `raw.githubusercontent.com` 读取。生成清单时下载地址前缀必须以 `/` 结尾，否则 Sparkle 会拼出缺少版本标签的失效地址。重跑发布前先查询 Release 是否已有同名资产；已有时直接做匿名下载验证，不得重复上传。
